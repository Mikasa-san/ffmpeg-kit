...
display_help() {
	...
	echo -e "\n'$COMMAND' builds FFmpegKit for iOS platform. By default nine architectures (armv7, armv7s, arm64, arm64-mac-catalyst, arm64-simulator, arm64e, i386, x86-64 and x86-64-mac-catalyst) are enabled without any external libraries. Options can be used to disable architectures and/or enable external libraries. Please note that GPL libraries need --enable-gpl flag to be set explicitly. When compilation ends, libraries are created under the prebuilt folder.\n"
...
get_arch_specific_cflags() {
	...
	armv7s)
		...
		;;
	arm64)
		echo "-arch arm64 -target $(get_target) -mcpu=generic -DFFMPEG_KIT_ARM64"
		;;
	arm64-mac-catalyst)
		echo "-arch arm64 -target $(get_target) -mcpu=generic -DFFMPEG_KIT_ARM64_MAC_CATALYST -isysroot ${SDK_PATH} -isystem ${SDK_PATH}/System/iOSSupport/usr/include -iframework ${SDK_PATH}/System/iOSSupport/System/Library/Frameworks"
		;;
	arm64-simulator)
		echo "-arch arm64 -target $(get_target) -mcpu=generic -DFFMPEG_KIT_ARM64_SIMULATOR"
		;;
	arm64e)
		echo "-arch arm64e -target $(get_target) -mcpu=generic -DFFMPEG_KIT_ARM64E"
		;;
	i386)
...
get_size_optimization_cflags() {
	...
	armv7 | armv7s | arm64 | arm64e | *-mac-catalyst)
		echo "-Oz -Wno-ignored-optimization-argument"
		;;
	i386 | x86-64 | arm64-simulator)
		echo "-O2 -Wno-ignored-optimization-argument"
		;;
	esac
}
...
get_size_optimization_asm_cflags() {
	...
	jpeg | ffmpeg)
		...
		;;
	*)
		get_size_optimization_cflags "$1"
		;;
	esac
}
...
get_arch_specific_ldflags() {
	...
	armv7s)
		...
		;;
	arm64)
		echo "-arch arm64 -target $(get_target)"
		;;
	arm64-mac-catalyst)
		echo "-arch arm64 -target $(get_target) -isysroot ${SDK_PATH} -L${SDK_PATH}/System/iOSSupport/usr/lib -iframework ${SDK_PATH}/System/iOSSupport/System/Library/Frameworks"
		;;
	arm64-simulator)
		echo "-arch arm64 -target $(get_target)"
		;;
	arm64e)
		echo "-arch arm64e -target $(get_target)"
		;;
	i386)
...
set_toolchain_paths() {
	...
	export CC="$(xcrun --sdk "$(get_sdk_name)" -f clang 2>>"${BASEDIR}"/build.log)"
	export OBJC="$(xcrun --sdk "$(get_sdk_name)" -f clang 2>>"${BASEDIR}"/build.log)"
	export CXX="$(xcrun --sdk "$(get_sdk_name)" -f clang++ 2>>"${BASEDIR}"/build.log)"
