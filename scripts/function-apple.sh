...
disable_ios_architecture_not_supported_on_detected_sdk_version() {
	...
	x86-64-mac-catalyst)
		...
		;;
	arm64-mac-catalyst | arm64-simulator)
	...
	if [[ ${SUPPORTED} -ne 1 && -z ${BUILD_FORCE} ]]; then
	...
	for FFMPEG_LIB in "${FFMPEG_LIBS[@]}"; do
		...
		if [[ "${FFMPEG_LIB}" == "libavcodec" ]]; then
			...
			for custom_library_index in "${CUSTOM_LIBRARIES[@]}"; do
				...
				library_name_uppercase=$(echo "${!library_name}" | tr '[a-z]' '[A-Z]')
				relative_license_path="CUSTOM_LIBRARY_${custom_library_index}_LICENSE_FILE"
				destination_license_path="${FFMPEG_LIB_FRAMEWORK_RESOURCE_PATH}/LICENSE.${library_name_uppercase}"
...
get_framework_directory() {
	...
	"${ARCH_VAR_IOS}") echo "bundle-apple-framework-ios${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONEOS}") echo "bundle-apple-framework-iphoneos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONESIMULATOR}") echo "bundle-apple-framework-iphonesimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MAC_CATALYST}") echo "bundle-apple-framework-mac-catalyst${LTS_POSTFIX}" ;;
	"${ARCH_VAR_TVOS}") echo "bundle-apple-framework-tvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVOS}") echo "bundle-apple-framework-appletvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVSIMULATOR}") echo "bundle-apple-framework-appletvsimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MACOS}") echo "bundle-apple-framework-macos${LTS_POSTFIX}" ;;
	esac
}
...
get_universal_library_directory() {
	...
	"${ARCH_VAR_IOS}") echo "bundle-apple-universal-ios${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONEOS}") echo "bundle-apple-universal-iphoneos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONESIMULATOR}") echo "bundle-apple-universal-iphonesimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MAC_CATALYST}") echo "bundle-apple-universal-mac-catalyst${LTS_POSTFIX}" ;;
	"${ARCH_VAR_TVOS}") echo "bundle-apple-universal-tvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVOS}") echo "bundle-apple-universal-appletvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVSIMULATOR}") echo "bundle-apple-universal-appletvsimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MACOS}") echo "bundle-apple-universal-macos${LTS_POSTFIX}" ;;
	esac
}
...
get_apple_architecture_variant() {
	...
	case ${ARCHITECTURE_VARIANT} in
	"${ARCH_VAR_IOS}") echo "ios${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONEOS}") echo "iphoneos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_IPHONESIMULATOR}") echo "iphonesimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MAC_CATALYST}") echo "mac-catalyst${LTS_POSTFIX}" ;;
	"${ARCH_VAR_TVOS}") echo "tvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVOS}") echo "appletvos${LTS_POSTFIX}" ;;
	"${ARCH_VAR_APPLETVSIMULATOR}") echo "appletvsimulator${LTS_POSTFIX}" ;;
	"${ARCH_VAR_MACOS}") echo "macos${LTS_POSTFIX}" ;;
	esac
}
...
get_cmake_osx_architectures() {
	case ${ARCH} in
	arm64 | arm64-simulator | arm64-mac-catalyst)
		echo "arm64"
		;;
	arm64e)
		echo "arm64e"
...
build_info_plist() {
	...
	tvos)
		MINIMUM_VERSION_KEY="MinimumOSVersion"
		MINIMUM_OS_VERSION="${TVOS_MIN_VERSION}"
...
get_sdk_name() {
	...
	x86-64)
		case ${FFMPEG_KIT_BUILD_TYPE} in
		ios) echo "iphonesimulator" ;;
		tvos) echo "appletvsimulator" ;;
		macos) echo "macosx" ;;
		esac
		;;
	i386)
...
get_min_version_cflags() {
	...
	arm64)
		case ${FFMPEG_KIT_BUILD_TYPE} in
		ios) echo "-miphoneos-version-min=$(get_min_sdk_version)" ;;
		tvos) echo "-mappletvos-version-min=$(get_min_sdk_version)" ;;
		macos) echo "-mmacosx-version-min=$(get_min_sdk_version)" ;;
		esac
		;;
	x86-64)
		case ${FFMPEG_KIT_BUILD_TYPE} in
		ios) echo "-mios-simulator-version-min=$(get_min_sdk_version)" ;;
...
create_libiconv_system_package_config() {
	...
	if [[ -n "$DETECTED_IOS_SDK_VERSION" && $(compare_versions "$DETECTED_IOS_SDK_VERSION" "17.0") -ge 0 ]] ||
		...
		_REQUIRES_LIBS="-liconv"
	else
		_REQUIRES_LIBS="-liconv -lcharset"
	fi
